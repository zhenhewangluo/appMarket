...
class CaiJiXia
{
	...
	function IB($rs)
	{
        ...
        if(!empty($this->vo['writer'])) $rs['writer'] = $this->vo['writer']; else $rs['writer'] = $this->GV('dwriter');
        if(!empty($this->vo['source'])) $rs['source'] = $this->vo['source']; else $rs['source'] = $this->GV('dsource');
	...
	function cjxxml(){
		...
		if($this->GV('ggmap')==0){
			...
			foreach($t as $k=>$r){
				$class = $k%2==0?'class="high"':'';
				$s .= '<tr '.$class.'>
				<td><a href="'.$r[0].'">'.$r[0].'</a></td>
				<td>'.$r[1].'</td>
				<td>Daily</td>
				<td>'.date("Y-m-d H:i",$r[2]).'</td>
			</tr>';	
			}
			$s .= $this->DC('xmlfooter');
			$s = str_replace(array('#ch#'),array('utf-8'),$s);
			$this->WF($xml,$s);
		}
		if($this->GV('bdmap')==0){
			@unlink($xml2);
