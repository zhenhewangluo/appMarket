 ...
 * @date	2011-12-08
 */
class BasedevpAction extends Action
{
	...
	private		$_urlPrefix = 'http://www.hjapk.com/UserCenter/index.php?';
	...
	protected function _initialize()
	{
		...
		if (is_array($this->_userinfo))
		{
			...
			if(isset($this->_userinfo['TeamHead']['path']))
			{
				$this->_userhead = explode(',',$this->_userinfo['TeamHead']['path']);
			}
			else
			{
...
//		Vendor('Ucenter.UcApi');  //载入UcApi扩展
		if(Cookie::is_set(C('COOKIE_NAME')))
		{
			...
			$usercookie = array();
			list($usercookie['uid'], $usercookie['teamname'], $usercookie['password'], $usercookie['email'], $usercookie['phone']) = explode("\t", Crypt::decrypt(Cookie::get(C('COOKIE_NAME')), C('DES_KEY'), 1));
			return $usercookie;
		}
		return 0;
	}
