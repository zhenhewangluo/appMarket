...
class AppsAction extends AppBaseAction
{
	...
	protected  $_layout = 'Layout:layout';
	...
	* ************************************************** */
	public function appList()
	{
		...
		$list = $appInfoModel->order("{$order}")->relation('AppCategory')
				...
				->select();
		//var_dump($list );
		$count = $appInfoModel->where("app_visible = '1' AND app_audit = '0'")->count();
		$cateObj = D('Category');
	...
	public function appauditList()
	{
		...
		if (!empty($list))
		{
			...
			if (!empty($catelistResult))
				...
				$show = $Page->show();
				$this->assign('applist', $list);
				$this->assign('auditpage', $show);
		}
		$this->assign('viewcontent', MODULE_NAME . ':appauditList');
		$this->display('appauditList');
	}
	/*	 * *********************************************
	 * Function:      appauditList
	* Description:    审核通过显示应用列表，
	...
	* ************************************************** */
	...
	protected function upload()
	{
		...
		else
		{
			...
			var_dump("C('APP_FILE_PATH') . $uploadList[0]['savename']");
...
//			//自动填
			...
			'source' => ($this->_usertype == 1 ? 'Admin' : 'Developercp'), //临时
//			
//			//推荐？？
