<?php if (!defined('THINK_PATH')) exit();?><div class="headbar">
	...
	<div class="operating">
		...
		<a href="javascript:void(0)">
			<button class="operating_btn" type="button" onclick="location.href='<?php echo U("Admin://Developer/appAuditRecycleList");?>'">
	...
	<table id="list_table" class="list_table">
		...
		<tbody>
			<?php if(is_array($applist)): $i = 0; $__LIST__ = $applist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><tr id="app_<?php echo ($vo['app_id']); ?>">
					...
					<td class="t_c">
						<a href="<?php echo U('Admin://Developer/appEdit?id='.$vo['app_id']);?>">
                    ...
                    <td class="t_c">
                    <a href="<?php echo U('Admin://Developer/appAudit?id='.$vo['app_id']);?>" >
							<img class="operator" src="../Public/images/icon_edit.gif" alt="审核" />
						</a>
                    </td>
...
function apps_del_one(id)
{
	$.dialog.confirm(
			'确定要删除所选中的信息吗？',
			function(){
				$.getJSON("<?php echo U('Admin://Developer/appsDel');?>", { id : id }, function(data){
					var tips = '';
					if(data.status == 1)
					{
						tips = 'succeed';
...
function apps_del()
{
	...
	$.dialog.confirm(
			...
			function(){
				...
				$.getJSON("<?php echo U('Admin://Developer/appsAuditDel');?>", {id:id}, function(data){
		//			if(data.status == 1)getDialog(data.msg, 'succeed');
					var tips = '';
					if(data.status == 1)
					{
