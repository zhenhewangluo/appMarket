...
/*		global $cookieName, $cookieKey;
		...
		_setcookie($cookieName, _authcode($uid . "\t" . $username . "\t" . $password . "\t" . $email, 'ENCODE', $cookieKey),30 * 24 * 3600);*/
		global $cookieName, $cookieKey;
		$uid = $get['uid'];
		$result = $GLOBALS['db']->fetch_first("SELECT * FROM `uc_members` WHERE `uid` = {$uid}");
		...
		header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
		_setcookie($cookieName, Crypt::encrypt("{$uid}\t{$username}\t{$password}\t{$email}\t{$phone}", $cookieKey, 1),30 * 24 * 3600);
	}
	...
	function synlogout($get, $post) {
		...
		//note 同步登出 API 接口
		header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
		_setcookie($cookieName, '', -86400 * 365);
	}
