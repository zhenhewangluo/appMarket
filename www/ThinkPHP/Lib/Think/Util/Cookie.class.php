...
class Cookie extends Think
{
    ...
    // 获取某个Cookie值
    static function get($name, $decode = 1) {
        $value   = $_COOKIE[C('COOKIE_PREFIX').$name];
        if($decode)$value   =  unserialize(base64_decode($value));
        return $value;
    }

    // 设置某个Cookie值
    static function set($name,$value,$expire='',$path='',$domain='',$encode=1) {
        if($expire=='') {
        ...
        if($encode)$value   =  base64_encode(serialize($value));
