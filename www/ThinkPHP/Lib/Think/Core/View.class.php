...
class View extends Think
{
    ...
    private function parseTemplateFile($templateFile) {
        if(''==$templateFile) {
			...
			//模板修改，后期需要修改layout可更改此处
			if(strlen(trim(C('APP_GROUP_LIST'))) > 0 && C('TMPL_FILE_DEPR') == '/' && (false !== strpos($templateFile,'Layout') || false !== strpos($templateFile,'success') || false !== strpos($templateFile,'error')))$count++;
