   ...
   /**
    ...
    private function _dispatch_jump($message,$status=1,$ajax=false)
    {
        ...
        if($status) { //发送成功信息
			...
			if(C('TMPL_JUMP_LAYOUT'))
			{
				$this->assign(C('TMPL_LAYOUT_LABEL'), C('TMPL_ACTION_SUCCESS'));
				$this->display(C('TMPL_LAYOUT_TPL'));
			}
			else
			{
            ...
            if(C('TMPL_JUMP_LAYOUT'))
			{
				$this->assign(C('TMPL_LAYOUT_LABEL'), C('TMPL_ACTION_ERROR'));
				$this->display(C('TMPL_LAYOUT_TPL'));
			}
			else
			{
