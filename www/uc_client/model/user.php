...
class usermodel {
	...
	function get_user_by_phone($username) {
		$arr = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."members WHERE username='$username'");
		return $arr;
	}
	...
	function add_user($username, $password, $email, $uid = 0, $questionid = '', $answer = '', $regip = '') {
		$password1 = md5($password);//回传
		$regip = empty($regip) ? $this->base->onlineip : $regip;
		$salt = substr(uniqid(rand()), -6);
		$password = md5(md5($password).$salt);
		$sqladd = $uid ? "uid='".intval($uid)."'," : '';
		$sqladd .= $questionid > 0 ? " secques='".$this->quescrypt($questionid, $answer)."'," : " secques='',";
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."members SET $sqladd username='$username', password='$password', email='$email', regip='$regip', regdate='".$this->base->time."', salt='$salt', password1='{$password1}'");//加字段
		$uid = $this->db->insert_id();
		$this->db->query("INSERT INTO ".UC_DBTABLEPRE."memberfields SET uid='$uid'");
		return $uid;
	}
	function reset_user_pwd($username, $newpw) {
        $data = $this->db->fetch_first("SELECT salt,password FROM ".UC_DBTABLEPRE."members WHERE username='$username'");
		if(md5(md5($newpw).$data['salt']) == $data['password']) return 1;
		$password1 = md5($newpw);//回传
        $this->db->query("UPDATE ".UC_DBTABLEPRE."members SET password='".md5(md5($newpw).$data['salt'])."', password1='$password1' WHERE username='$username'");
	...
	function edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw = 0, $questionid = '', $answer = '') {
		...
		if(!$ignoreoldpw && $data['password'] != md5(md5($oldpw).$data['salt'])) {//return $username;//md5($oldpw).$data['salt'];
			return -1;
		}
		$password1 = md5($newpw);//回传
		...
		$sqladd .= ($sqladd ? ',' : '')." password1='$password1'";//回传
		if($sqladd || $emailadd) {
