...
function uc_user_register($username, $password, $email, $questionid = '', $answer = '', $regip = '') {
	return call_user_func(UC_API_FUNC, 'user', 'register', array('username'=>$username, 'password'=>$password, 'email'=>$email, 'questionid'=>$questionid, 'answer'=>$answer, 'regip' => $regip));//加参数
}
 ...
 * @param $newpw 会员的新密码
 */
function uc_user_reset_pwd($username, $newpw) {
        return call_user_func(UC_API_FUNC, 'user', 'resetpwd', array('username'=>$username, 'newpw'=>$newpw));
}
