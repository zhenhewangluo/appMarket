...
class usercontrol extends uc_base {
			...
			if($this->user = $_ENV['user']->get_user_by_uid($uid)) {
				...
				foreach($this->cache['apps'] as $appid => $app) {
					if($app['synlogin'] && $app['appid'] != $this->app['appid']) {
						$synstr .= '<script type="text/javascript" src="'.$app['url'].'/api/uc.php?time='.$this->time.'&code='.urlencode($this->authcode('action=synlogin&username='.$this->user['username'].'&uid='.$this->user['uid'].'&password='.$this->user['password']."&email=".$this->user['email']."&time=".$this->time, 'ENCODE', $app['authkey'])).'"></script>';
					}
				}
				return $synstr;
			}
	...
	function onregister() {
		...
		$uid = $_ENV['user']->add_user($username, $password, $email, 0, $questionid, $answer, $regip);//加参数
...
function onresetpwd() {
        ...
        return $_ENV['user']->reset_user_pwd($username, $newpw);
}
	function onedit() {
		$this->init_input();
		$username = $this->input('username');
	...
	function onlogin() {
		...
		if($isuid == 1) {
			...
			$user = $_ENV['user']->get_user_by_phone($username);
		}
		else {
