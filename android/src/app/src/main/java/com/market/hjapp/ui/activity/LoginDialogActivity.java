...
public class LoginDialogActivity extends BaseActivity {
	...
	TaskResultListener mLoginListener = new TaskResultListener() {
		...
		public void onTaskResult(boolean success, HashMap<String, Object> result) {
			...
			if (success) {
				if (!GeneralUtil
						.hasEnteredUserGuidePageBefore(LoginDialogActivity.this)) {
					MyLog.d(TAG, "!hasEnteredUserGuidePageBefore");
					switch (GeneralUtil
							.getUserGuideFunc(LoginDialogActivity.this)) {
					case 0:
						Intent i = new Intent(getApplicationContext(),
								Lead2Activity.class);
						startActivity(i);
		...
		public void onClick(View arg0) {
			if (checkInput()) {
				try {
					...
					new LoginTask(LoginDialogActivity.this, mLoginListener)
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		MyLog.e("TAG", "LoginDialogActivity run.");
	...
	private void saveUserLog(int action) {
		// save user log
		GeneralUtil.saveUserLogType3(LoginDialogActivity.this, mPageNo, action);
		// if (action==0) {
