...
$sqlInfo = "select * from `_kid_android`.`upgrade`";
...
if ($row)
{
	...
	$sql = "select `app_version`,`version`,`apk_path` from `_kid_android`.`am_appinfo` a left join `_kid_android`.`am_app_device_type` b on a.app_id=b.app_id where a.`app_visible`=1 and a.`app_id`=".$row['appid'];
