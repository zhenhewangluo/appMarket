<?php
	/************************************************************
	 *   Interface 4
     *   获取某分类下应用列表  下载量排序  get_applist_favorite.php
	 *
     *   Author:  bluesie
	 *   Time:    2010-6-29
	 *	 paras  proto uid mid sid  cate start_no  length 	  
	 *************************************************************/
	require("../inc/init_interface.php");	
	
    	if($AM_CURRENT_REQUEST["PROTO"] != 4){
	...
	if($isflagexists){
		if($cate ==5){
			$cate = "59,62,64,66,67,68,69,74,75,76,77,78";
			$sqlCate = "select distinct app_id from am_app_category where cate_id in(".$cate.")";
		}else{
	...
	if($aCateId)
	{
		...
		$keyName = $AM_MEMCACHE["thunder_am"][0]."_cateappdown_".$start_no."_".$page_size."_".$cate."_".$current_device_id;
		if($AM_MEMCACHE["thunder_am"][2]){
			if(!($aCate = $memobj->get($keyName))) $isflagexists = 2;
		}else $isflagexists = 1;
		if($isflagexists){
			$strCate = join(",",$aCateId);
			$sqlCate = "select a.app_id,a.app_name,a.app_desc,a.app_version,a.version,b.pkg_name,a.app_price,b.app_size,a.app_rate_up,a.app_rate_down,a.author_name from am_appinfo a left join am_app_device_type b on a.app_id =b.app_id  where b.device_type_id=".$current_device_id." and a.app_id in (".$strCate.") order by a.app_downloads desc ";
