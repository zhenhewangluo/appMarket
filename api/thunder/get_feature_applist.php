<?php
	/************************************************************
	 *   Interface 8
     *   获取专题应用列表  get_feature_keyword.php  广告 大图
	 *
     *   Author:  bluesie
	 *   Time:    2010-6-29
	...
	//paras
	$limit = intval(__getPost('length'));
	if($limit < 0)
		$limit = 0;
	//appid

	$isflagexists = 0; $aCateId = array();
	$keyName = $AM_MEMCACHE["thunder_am"][0]."_am_homepage_ads";
	if($AM_MEMCACHE["thunder_am"][2]){
		if(!($aCateId = $memobj->get($keyName))) $isflagexists = 2;
	}else $isflagexists = 1;
	if($isflagexists){
		$sqlCate = "select distinct typeid  from am_rec_daily_day  where visible =1 order by edittime desc limit 20";
		$rsCate = mysql_query($sqlCate, $conn);
		while ($rowCate = mysql_fetch_assoc($rsCate)){
			$aCateId[] = $rowCate['typeid'];
		}
	...
	if($aCateId)
	{
		...
		$keyName = $AM_MEMCACHE["thunder_am"][0]."_apphomepage_".$limit."_".$current_device_id;
		if($AM_MEMCACHE["thunder_am"][2]){
