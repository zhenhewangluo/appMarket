<?php
	/************************************************************
	 *   Interface 6
     *   按照关键词获取应用列表  按时间排序   get_applist_keyword.php
	 *
     *   Author:  bluesie
	 *   Time:    2010-6-29
	 *	 paras  proto uid mid sid  word start_no  length 	  
	 *************************************************************/
	...
	$keyword = stopSql(__getPost("word"));
	if(!$keyword)
	{
	...
	$keyName = $AM_MEMCACHE["thunder_am"][0]."_keyword_".$start_no."_".$page_size."_".md5($keyword)."_".$current_device_id;
	if($AM_MEMCACHE["thunder_am"][2]){
		if(!($aCate = $memobj->get($keyName))) $isflagexists = 2;
	}else $isflagexists = 1;
	if($isflagexists){
		$sqlCate = "select a.app_id,a.app_name,a.app_desc,a.app_version,a.version,b.pkg_name,a.app_price,b.app_size,a.app_rate_up,a.app_rate_down,a.author_name from am_appinfo a left join am_app_device_type b on a.app_id =b.app_id  where b.device_type_id=".$current_device_id." and  (a.app_name like  '%".$keyword."%'  or a.app_slogan like  '%".$keyword."%' or a.app_desc like '%".$keyword."%' or a.author_name like '%".$keyword."%' )  ";
