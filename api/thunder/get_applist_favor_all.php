<?php
	/************************************************************
	 *   Interface 10
     *   获取全部最热下载应用列表 get_applist_favor_all.php
	 *
     *   Author:  bluesie
	 *   Time:    2010-6-29
	...
	require("../inc/init_interface.php");	
    	...
    	if($AM_CURRENT_REQUEST["PROTO"] != 10){
	...
	$keyName = $AM_MEMCACHE["thunder_am"][0]."_cateappdown_".$start_no."_".$page_size."_".$current_device_id;
	if($AM_MEMCACHE["thunder_am"][2]){
		if(!($aCate = $memobj->get($keyName))) $isflagexists = 2;
	}else $isflagexists = 1;
	if($isflagexists){
		//info
		$sqlCate = "select a.app_id,a.app_name,a.app_desc,a.app_version,a.version,b.pkg_name,a.app_price,b.app_size,a.app_rate_up,a.app_rate_down,a.author_name from am_appinfo a left join am_app_device_type b on a.app_id =b.app_id  where b.device_type_id=".$current_device_id."  order by a.app_downloads desc ";
