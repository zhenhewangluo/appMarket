<?php
	...
	while ($row = mysql_fetch_assoc($result)){
		...
		$arrAll[] = $row;
		/*$sql4 = "update `smarterspider`.`smarterspider` set `status`='tested' where id=".$row['id'];
	...
	for($i=0; $i<count($arrAmauthor); $i++){
		$sql = "select id from am_author where group_name='".$arrAmauthor[$i]['group_name']."'";
		$res = mysql_query($sql, $conn);
		$row = mysql_fetch_assoc($res);
		if($row['id']){
			$arrAuthor[$arrAmauthor[$i]['group_name']] = $row['id'];
		}else{		
			$sql = "insert into am_author set contact_email='".$arrAmauthor[$i]['contact_email']."',group_name='".$arrAmauthor[$i]['group_name']."',website='".$arrAmauthor[$i]['website']."',phone='".$arrAmauthor[$i]['phone']."'";	
