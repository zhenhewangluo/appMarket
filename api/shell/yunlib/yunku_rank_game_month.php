...
if(!empty($_SESSION['uid'])&&!empty($_SESSION['username'])){
	...
	$conn = connect_db();
	$get_path=$_SERVER['AM_SITE_ROOT'].'data/yunlib_rank_game.txt';
	$content=file_get_contents($get_path);
	if($content){
		$contents=json_decode($content,true);
		$sum=count($contents['yunlib_month']);
		foreach($contents['yunlib_month'] as $key=>$val){
			$sec[$key] = $val; 
		}
	}else{
		$contents['yunlib_month']=array();
...
if($action=="add"){
		...
		$total=count($contents['yunlib_month']);
		if(in_array($_POST["app_id"],$contents['yunlib_month'])){
			$msg = "该应用ID已存在，请重试！";
		}else{
			...
			array_splice($contents['yunlib_month'],$real_pos,0,$app_id);
...
if($action=="edit"){
		...
		if($app_sort==$edit_sort){
			...
			echo $edit_sort."<br>";
			$contents['yunlib_month'][$edit_sort]=$_POST["app_id"];
		
		}else{
			$edit_sort=$edit_sort-1;
			unset($contents['yunlib_month'][$edit_sort]);
			$contents['yunlib_month']=array_merge($contents['yunlib_month']);
			$total=count($contents['yunlib_month']);
			if(isset($app_sort)&&$app_sort>$total)$real_pos=$app_sort;
			if(isset($app_sort)&&$app_sort<=$total)$real_pos=$app_sort-1;
			if(empty($app_sort))$real_pos=$total+1;
		
			array_splice($contents['yunlib_month'],$real_pos,0,$e_id);
		}
...
if($msg) echo "<script>alert('".$msg."');window.location.href='./yunku_rank_game_month.php'</script>";
//add
?>
...
<?php
	if(is_array($contents['yunlib_month'])){
?>
	<table border="1">
	<tr>
...
<?php
	foreach($contents['yunlib_month'] as $key=>$row){	
