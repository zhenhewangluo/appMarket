...
if(!empty($_SESSION['uid'])&&!empty($_SESSION['username'])){
	...
	if($content){
		...
		$sum=count($contents['yunlib_week']);
		foreach($contents['yunlib_week'] as $key=>$val){
			$sec[$key] = $val; 
		}
	}else{
		$contents['yunlib_week']=array();
...
if($action=="add"){
		...
		$total=count($contents['yunlib_week']);
		if(in_array($_POST["app_id"],$contents['yunlib_week'])){
			$msg = "该应用ID已存在，请重试！";
		}else{
			...
			array_splice($contents['yunlib_week'],$real_pos,0,$app_id);
...
if($action=="edit"){
		...
		if($app_sort==$edit_sort){
			...
			echo $edit_sort."<br>";
			$contents['yunlib_week'][$edit_sort]=$_POST["app_id"];
		
		}else{
			$edit_sort=$edit_sort-1;
			unset($contents['yunlib_week'][$edit_sort]);
			$contents['yunlib_week']=array_merge($contents['yunlib_week']);
			$total=count($contents['yunlib_week']);
			if(isset($app_sort)&&$app_sort>$total)$real_pos=$app_sort;
			if(isset($app_sort)&&$app_sort<=$total)$real_pos=$app_sort-1;
			if(empty($app_sort))$real_pos=$total+1;
		
			array_splice($contents['yunlib_week'],$real_pos,0,$e_id);
		}
...
if($msg) echo "<script>alert('".$msg."');window.location.href='./yunku_rank_game_week.php'</script>";
//add
?>
...
<?php
	if(is_array($contents['yunlib_week'])){
?>
	<table border="1">
	<tr>
...
<?php
	foreach($contents['yunlib_week'] as $key=>$row){	
