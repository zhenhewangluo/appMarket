...
if(!empty($_SESSION['uid'])&&!empty($_SESSION['username'])){
	...
	$conn = connect_db();
	$get_path=$_SERVER['AM_SITE_ROOT'].'data/yunlib_category_cate.txt';
...
if($msg){
	echo "<script>alert('".$msg."');window.location.href='yunku_category_game.php?model=".$topic."'</script>";
	exit;
}
//add
?>

选择分类：<select name="model" id="model" style="width:100px;" onchange="show()" >
		<option value="" <?php if(empty($topic)){?>selected="selected"<?php }?>>选择分类</option>

      
<?php
$sql="SELECT id,name FROM am_category where parent_id=6";
$query = mysql_query($sql,$conn);
			...
			url: 'yunku_ajax.php',
					...
					success: function(msg) {
						if(msg){
							...
							if(msg.id!=cateid){
								html='<font color="#FF0000">此应用不属于当前分类</font>';
								$("#sub").hide();
							}else{
								$("#sub").show();
							}
...
<input type="hidden"  value="<?php echo $topic?>" id="cateid">
