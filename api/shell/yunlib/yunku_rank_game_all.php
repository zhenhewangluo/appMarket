...
if(!empty($_SESSION['uid'])&&!empty($_SESSION['username'])){
	...
	if($content){
		...
		$sum=count($contents['yunlib_all']);
		foreach($contents['yunlib_all'] as $key=>$val){
			$sec[$key] = $val; 
		}
	}else{
		$contents['yunlib_all']=array();
...
if($action=="add"){
		...
		if(in_array($_POST["app_id"],$contents['yunlib_all'])){
			$msg = "该应用ID已存在，请重试！";
		}else{
			...
			array_splice($contents['yunlib_all'],$real_pos,0,$app_id);
...
if($action=="edit"){
		...
		if($app_sort==$edit_sort){
			...
			echo $edit_sort."<br>";
			$contents['yunlib_all'][$edit_sort]=$_POST["app_id"];
		
		}else{
			$edit_sort=$edit_sort-1;
			unset($contents['yunlib_all'][$edit_sort]);
			$contents['yunlib_all']=array_merge($contents['yunlib_all']);
			$total=count($contents['yunlib_all']);
			if(isset($app_sort)&&$app_sort>$total)$real_pos=$app_sort;
			if(isset($app_sort)&&$app_sort<=$total)$real_pos=$app_sort-1;
			if(empty($app_sort))$real_pos=$total+1;
		
			array_splice($contents['yunlib_all'],$real_pos,0,$e_id);
		}
...
if($msg) echo "<script>alert('".$msg."');window.location.href='./yunku_rank_game_all.php'</script>";
//add
?>
...
<?php
	if(is_array($contents['yunlib_all'])){
?>
	<table border="1">
	<tr>
...
<?php
	foreach($contents['yunlib_all'] as $key=>$row){	
